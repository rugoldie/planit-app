    ...
    return (
      <div style={{ position:"absolute", inset:0, pointerEvents:"none" as const }}>
        <svg style={{ position:"absolute", inset:0, width:"100%", height:"100%" }} xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="bp-sm-ev" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#4da6ff" strokeWidth="0.4" opacity="0.35"/>
            </pattern>
            <pattern id="bp-lg-ev" width="200" height="200" patternUnits="userSpaceOnUse">
              <rect width="200" height="200" fill="url(#bp-sm-ev)"/>
              <path d="M 200 0 L 0 0 0 200" fill="none" stroke="#4da6ff" strokeWidth="0.9" opacity="0.4"/>
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#bp-lg-ev)"/>
...
const EventView = () => {
  ...
  const renderPolls = () => polls.length === 0 ? null : (
    <div className="mt-3 space-y-3">
      {polls.map(poll => (
        <div key={poll.id} className="relative rounded-2xl p-4" style={{ backgroundColor: isLightBg ? "rgba(0,0,0,0.05)" : "rgba(255,255,255,0.10)", border: `1px solid ${isLightBg ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.14)"}` }}>
  ...
  return (
    <div className="flex flex-col min-h-screen pb-28" style={{ backgroundColor: containerBg }}>
      {isVintage ? (
        ...
        <>
          {(() => {
            ...
            const customSubtitleSz = event.text_size === "Small" ? "9px" : event.text_size === "Large" ? "13px" : "11px";
            const customLabelSz = event.text_size === "Small" ? "8px" : event.text_size === "Large" ? "10px" : "9px";
            const customValueSz = event.text_size === "Small" ? "12px" : event.text_size === "Large" ? "16px" : "14px";
            const customHostedBySz = event.text_size === "Small" ? "10px" : event.text_size === "Large" ? "13px" : "11px";
            ...
            try {
              ...
              if (Array.isArray(parsed)) { stickerItems = parsed; }
                ...
                stickerItems = parsed.items || [];
                const cl = parsed.customLayout as string;
                customLayout = (cl === "centred" ? "cards" : cl === "ocean" ? "ocean" : cl === "editorial" ? "editorial" : "cards") as "cards" | "editorial" | "ocean";
                bubbleStyleVal = parsed.bubbleStyle || "frosted";
              }
            } catch {}
            const infoBubbleStyle: React.CSSProperties = bubbleStyleVal === "solid"
              ...
              : bubbleStyleVal === "outlined"
                ? { borderRadius:"20px",border:`2px solid ${accentColor}`,padding:"16px 18px",backgroundColor:"transparent" }
                : { borderRadius:"20px",border:`1px solid ${frostBorder}`,padding:"16px 18px",backgroundColor:frostBg,backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)" };
            const infoTextColor = bubbleStyleVal === "solid" ? `hsl(${event.bubble_text_color || "0 0% 0%"})` : tCol;
            const infoLabelColor = bubbleStyleVal === "solid" ? `hsl(${event.bubble_text_color || "0 0% 0%"})` : accentColor;
            return (
              <div style={{ minHeight: "100vh", position: "relative", overflow: "hidden", ...patStyle }}>
                ...
                <div className="flex items-center justify-between px-5 pt-5 relative z-10">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={copyCode} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold" style={{ backgroundColor: frostBg, border: `1px solid ${frostBorder}`, color: accentColor, fontFamily: "'Inter', sans-serif", backdropFilter: "blur(8px)" }}>
                      {codeCopied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                      {event.code}
                    </button>
                    <button onClick={() => setShowDMs(true)} className="flex flex-col items-center gap-0.5">
                ...
                {customLayout === "ocean" ? (
                  <div className="relative z-10 pb-6">
                    <div className="relative z-10 px-5 pt-6 pb-2 text-center">
                      ...
                      <h1 style={{ fontFamily:eventFontFamily,fontSize:titleSz,fontWeight:900,color:tCol,lineHeight:1.1,marginBottom:"8px" }}>{event.title||"Untitled Event"}</h1>
                      {event.vibe && <p style={{ fontFamily:"'Inter',sans-serif",fontSize:"13px",fontStyle:"italic",color:accentColor,opacity:0.6,marginBottom:"6px" }}>{event.vibe}</p>}
                      <p style={{ fontFamily:"'Inter',sans-serif",fontSize:"11px",color:tMuted,marginBottom:"16px" }}>hosted by {profile?.name||"Host"}</p>
                      <svg viewBox="0 0 320 20" style={{ width:"100%",maxWidth:"280px",margin:"0 auto",display:"block",marginBottom:"20px" }}>
                    ...
                    <div className="flex flex-col gap-3 px-5">
                      {isHost && rsvpDeadline && <div className="w-full rounded-xl px-4 py-3 flex items-start gap-2 text-left" style={{ backgroundColor:"rgba(249,115,22,0.12)",border:"1px solid rgba(249,115,22,0.3)" }}><span style={{ color:"#f97316",fontSize:"16px" }}>⏰</span><div><p className="text-sm font-semibold" style={{ color:"#f97316" }}>{deadlinePassed?"RSVP deadline has passed":`RSVP deadline: ${rsvpDeadline.toLocaleDateString(undefined,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}`}</p>{noReplyCount>0&&<p className="text-xs mt-0.5" style={{ color:"rgba(249,115,22,0.7)" }}>{noReplyCount} guest{noReplyCount>1?"s":""} {deadlinePassed?"didn't reply":"haven't replied"}</p>}</div></div>}
                      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px" }}>
                        <div style={{ ...infoBubbleStyle,borderRadius:"16px",padding:"14px 8px",textAlign:"center" as const }}><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"7px",fontWeight:700,letterSpacing:"0.22em",textTransform:"uppercase" as const,color:infoLabelColor,marginBottom:"4px" }}>Day</p><span style={{ fontFamily:eventFontFamily,fontSize:"34px",fontWeight:900,color:infoTextColor,lineHeight:1,display:"block" }}>{dayNum||"—"}</span><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"9px",fontWeight:700,color:infoLabelColor,textTransform:"uppercase" as const,marginTop:"2px" }}>{monthName||"TBD"}</p></div>
                        <div style={{ ...infoBubbleStyle,borderRadius:"16px",padding:"14px 8px",textAlign:"center" as const }}><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"7px",fontWeight:700,letterSpacing:"0.22em",textTransform:"uppercase" as const,color:infoLabelColor,marginBottom:"4px" }}>Time</p><span style={{ fontFamily:eventFontFamily,fontSize:eventDate?"16px":"28px",fontWeight:900,color:infoTextColor,lineHeight:1.1,display:"block" }}>{timeStr||"—"}</span><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"9px",fontWeight:700,color:infoLabelColor,textTransform:"uppercase" as const,marginTop:"2px" }}>{dayOfWeek?dayOfWeek.slice(0,3).toUpperCase():"TBD"}</p></div>
                        <div style={{ ...infoBubbleStyle,borderRadius:"16px",padding:"14px 8px",textAlign:"center" as const }}><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"7px",fontWeight:700,letterSpacing:"0.22em",textTransform:"uppercase" as const,color:infoLabelColor,marginBottom:"4px" }}>Going</p><span style={{ fontFamily:eventFontFamily,fontSize:"34px",fontWeight:900,color:infoTextColor,lineHeight:1,display:"block" }}>{goingList.length}</span><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"9px",fontWeight:700,color:infoLabelColor,textTransform:"uppercase" as const,marginTop:"2px" }}>Guests</p></div>
                      </div>
                      {event.location && <div style={{ ...infoBubbleStyle,borderRadius:"50px",padding:"14px 22px" }}><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"8px",fontWeight:700,letterSpacing:"0.18em",textTransform:"uppercase" as const,color:infoLabelColor,marginBottom:"3px" }}>📍 Location</p><p style={{ fontFamily:eventFontFamily,fontSize:"14px",fontWeight:600,color:infoTextColor }}>{event.location}</p></div>}
                      {event.dress_code && <div style={{ ...infoBubbleStyle,borderRadius:"50px",padding:"14px 22px" }}><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"8px",fontWeight:700,letterSpacing:"0.18em",textTransform:"uppercase" as const,color:infoLabelColor,marginBottom:"3px" }}>👗 Dress Code</p><p style={{ fontFamily:eventFontFamily,fontSize:"14px",fontWeight:600,color:infoTextColor }}>{event.dress_code}</p></div>}
                      ...
                      <div style={{ borderRadius:"20px",border:`1px solid ${frostBorder}`,padding:"16px 18px",backgroundColor:frostBg,backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)" }}>
                        <div className="flex items-center justify-between mb-3"><p style={{ fontFamily:"'Inter',sans-serif",fontSize:"8px",fontWeight:700,letterSpacing:"0.22em",textTransform:"uppercase" as const,color:accentColor }}>Gallery</p><button onClick={()=>photoInput.current?.click()} className="text-xs font-bold rounded-full px-3 py-1" style={{ backgroundColor:accentColor,color:accentText,fontFamily:"'Inter',sans-serif" }}>Add photo</button><input ref={photoInput} type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} /></div>
                        ...
                        {photos.length===0&&!uploadingPhoto?<p style={{ fontFamily:"'Inter',sans-serif",fontSize:"12px",color:tMuted,textAlign:"center",padding:"16px 0" }}>No photos yet — add the first one!</p>:<div className="grid grid-cols-3 gap-1.5">{photos.map((p)=>(<div key={p.id} className="aspect-square rounded-xl overflow-hidden cursor-pointer" onClick={()=>setSelectedPhoto(p)}><img src={p.photo_url} alt="" className="w-full h-full object-cover" /></div>))}</div>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col relative z-10 pb-6">
                    <div className="flex flex-col px-6 pt-6">
                      {event.vibe && <span style={{ fontFamily:"'Inter',sans-serif",fontSize:customSubtitleSz,fontWeight:700,letterSpacing:"0.18em",textTransform:"uppercase" as const,color:accentColor,marginBottom:"8px",display:"block" }}>{event.vibe}</span>}
                      {isHost && rsvpDeadline && <div className="w-full mb-3 rounded-xl px-4 py-3 flex items-start gap-2" style={{ backgroundColor:"rgba(249,115,22,0.12)",border:"1px solid rgba(249,115,22,0.3)" }}><span style={{ color:"#f97316",fontSize:"16px" }}>⏰</span><div><p className="text-sm font-semibold" style={{ color:"#f97316" }}>{deadlinePassed?"RSVP deadline has passed":`RSVP deadline: ${rsvpDeadline.toLocaleDateString(undefined,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}`}</p>{noReplyCount>0&&<p className="text-xs mt-0.5" style={{ color:"rgba(249,115,22,0.7)" }}>{noReplyCount} guest{noReplyCount>1?"s":""} {deadlinePassed?"didn't reply":"haven't replied"}</p>}</div></div>}
                      <h1 style={{ fontFamily:eventFontFamily,fontSize:titleSz,fontWeight:900,color:tCol,lineHeight:0.95,marginBottom:"10px",textShadow:isLight?"none":"0 2px 12px rgba(0,0,0,0.5)" }}>{event.title||"Untitled Event"}</h1>
                      <p style={{ fontFamily:"'Inter',sans-serif",fontSize:customHostedBySz,color:tMuted,marginBottom:"14px" }}>hosted by {profile?.name||"Host"}</p>
                      <div style={{ width:"48px",height:"3px",backgroundColor:accentColor,marginBottom:"16px",borderRadius:"2px" }} />
                      {eventDate && <p style={{ fontFamily:"'Inter',sans-serif",fontSize:"14px",color:tCol,marginBottom:"16px",fontWeight:500 }}>📅 {dayNum} {monthName} · {timeStr}</p>}
                    </div>
                    <div className="w-full flex flex-col gap-3 px-5 mt-1">
                  ...
                  <div className="flex flex-col items-center text-center relative z-10 pb-6">
                    <div className="flex flex-col items-center text-center px-5 pt-6">
                      {event.vibe && <div style={{ border:`1px solid ${accentColor}50`,borderRadius:"50px",padding:"3px 12px",backgroundColor:`${accentColor}15`,backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",marginBottom:"10px",display:"inline-block" }}><span style={{ fontFamily:"'Inter',sans-serif",fontSize:customSubtitleSz,fontWeight:700,letterSpacing:"0.18em",textTransform:"uppercase" as const,color:accentColor }}>{event.vibe}</span></div>}
                      ...
                      <p style={{ fontFamily:"'Inter',sans-serif",fontSize:customHostedBySz,color:tMuted,marginBottom:"18px" }}>hosted by {profile?.name||"Host"}</p>
                      ...
                      {eventDate && <div style={{ borderRadius:"50px",border:`1px solid ${frostBorder}`,padding:"8px 18px",backgroundColor:frostBg,backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",marginBottom:"18px",display:"inline-block" }}><span style={{ fontFamily:eventFontFamily,fontSize:customValueSz,fontWeight:titleWt,color:accentColor }}>{dayNum} {monthName}<span style={{ color:tMuted,margin:"0 8px" }}>·</span>{timeStr}<span style={{ color:tMuted,margin:"0 8px" }}>·</span>{goingList.length} going</span></div>}
                    </div>
      ...
      {/* Shared content area */}
      {!isVintage && !isGalaxy && !isSunny && !isMidnight && !isOcean && !isBlush && !isForest && !isCustom && (
        <div className="px-5 pt-4">
          {/* Who's going section */}
      ...
      <div className="fixed bottom-4 left-4 right-4 z-50">
        {barMinimised && rsvp ? (
          <button
            ...
            style={{ backgroundColor: "rgba(56,56,56,0.95)", color: "#2563eb" }}
          >
            {rsvpLabel}
          </button>
        ) : (
            ...
            <>
              ...
              <div className="px-4 py-3 border-t border-border flex gap-2">
                <input
                  ...
                  enterKeyHint="send" onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); sendDM(); } }}
                  placeholder="Type a reply..."
                  className="flex-1 rounded-full px-4 py-2.5 text-sm text-white placeholder:text-muted-foreground outline-none border border-border"
                  style={{ backgroundColor: "#383838" }}
                />
                <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); sendDM(); }} className="w-11 h-11 rounded-full bg-primary flex items-center justify-center">
                  <Send className="w-4 h-4 text-primary-foreground" />
                </button>
              </div>
            </>
          ...
          <div className="px-5 pt-5 pb-3 border-b border-border shrink-0">
            ...
            <div className="flex gap-2 mt-3">
              {(["going", "waitlist"] as const).map(tab => (
                <button key={tab} onClick={() => setGuestListTab(tab)} className="px-4 py-1.5 rounded-full text-sm font-semibold" style={{ background: guestListTab === tab ? "#2563eb" : "#1e1e1e", color: guestListTab === tab ? "#ffffff" : "#fff", border: "none" }}>
                  {tab === "going"
                ...
                : <div className="space-y-2">
                    {waitlistProfiles.map(p => (
                      <div key={p.user_id} className="flex items-center gap-3 bg-card rounded-2xl px-4 py-3 border border-border">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => approveWaitlist(p.user_id)} className="text-xs font-bold px-3 py-1.5 rounded-full" style={{ background: "#2563eb", color: "#ffffff", border: "none" }}>Approve</button>
      ...
      <Drawer open={showRsvpDeadlineSheet} onOpenChange={setShowRsvpDeadlineSheet}>
        <DrawerContent className="bg-background border-t border-border">
          <div className="px-5 pt-5 pb-8">
            ...
            <button onClick={saveRsvpDeadline} className="w-full py-3.5 rounded-xl text-sm font-bold" style={{ background: "#2563eb", color: "#ffffff", border: "none" }}>Save deadline</button>
          </div>
        </DrawerContent>
      </Drawer>
