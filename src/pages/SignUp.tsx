...
const SignUp = () => {
  ...
  const passwordStrength = useMemo(() => {
    ...
    return { label: "strong", percent: 100, color: "#aaee44" };
        ...
        {step === 1 && (
          <div className="bg-card rounded-[var(--radius)] p-8 w-full max-w-xs border border-border">
            ...
            <ul className="mt-4 space-y-1.5 text-sm">
              <li className="flex items-center gap-2" style={{ color: hasMinLength ? "#aaee44" : "#999" }}>
                {hasMinLength ? <Check className="w-4 h-4" /> : <span className="w-4 h-4 inline-block" />}
                At least 6 characters
              </li>
              <li className="flex items-center gap-2" style={{ color: hasNumber ? "#aaee44" : "#999" }}>
