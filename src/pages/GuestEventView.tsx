        ...
        <>
          <div style={{ backgroundColor: "#f5f0e8" }}>
            ...
            <div className="px-5 mt-4">
              <div className="rounded-2xl p-4" style={{ backgroundColor: "#2c1810" }}>
                <button onClick={() => setGuestListExpanded(!guestListExpanded)} className="flex items-center justify-between w-full mb-3">
                  <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "14px", fontWeight: 700, color: "#f5f0e8" }}>Who's going</h2>
                  <div className="flex items-center gap-2">
                    {goingList.length > 0 && <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", fontWeight: 700, color: "#8b7355" }}>{goingList.length} going</span>}
                    {guestListExpanded ? <ChevronUp className="w-4 h-4" style={{ color: "#8b7355" }} /> : <ChevronDown className="w-4 h-4" style={{ color: "#8b7355" }} />}
                  </div>
                </button>
                {!guestListExpanded ? (
                  <>
                    <div className="flex items-center gap-2 overflow-x-auto mb-2">
                      {goingList.length === 0 && <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", color: "#8b7355" }}>No one yet</p>}
                      {goingList.map((r, i) => (
                        <div key={i} className="flex flex-col items-center shrink-0">
                          <div className="w-10 h-10 rounded-full flex items-center justify-center overflow-hidden" style={{ border: "2px solid #8b7355", backgroundColor: "#3d2a1a" }}>
                            {r.avatar_url ? <img src={r.avatar_url} alt="" className="w-full h-full object-cover" /> : <span className="text-xs font-bold" style={{ color: "#f5f0e8" }}>{getInitials(r.name)}</span>}
                          </div>
                          <span className="mt-1 max-w-[40px] truncate" style={{ fontFamily: "'Playfair Display', serif", fontSize: "10px", color: "#8b7355" }}>{r.name.split(" ")[0]}</span>
                        </div>
                      ))}
                    </div>
                    {maybeList.length > 0 && (
                        ...
                        {maybeList.map((r, i) => (
                          <div key={i} className="flex flex-col items-center shrink-0">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center overflow-hidden opacity-60" style={{ backgroundColor: "#3d2a1a" }}>
                              {r.avatar_url ? <img src={r.avatar_url} alt="" className="w-full h-full object-cover" /> : <span style={{ fontSize: "10px", fontWeight: 700, color: "#8b7355" }}>{getInitials(r.name)}</span>}
                            </div>
                            <span className="mt-0.5 max-w-[36px] truncate" style={{ fontFamily: "'Playfair Display', serif", fontSize: "9px", color: "#8b7355" }}>{r.name.split(" ")[0]}</span>
                          </div>
                        ))}
                        <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "10px", fontWeight: 600, color: "#8b7355" }}>{maybeList.length} maybe</span>
                      </div>
                    )}
                    {rsvp === "yes" && (
                      <div className="mt-3 flex justify-center">
                        <div className="rounded-full px-4 py-1.5" style={{ fontSize: "12px", fontWeight: 700, fontFamily: "'Playfair Display', serif", backgroundColor: "rgba(139,115,85,0.15)", color: "#8b7355" }}>
                          ✦ You're going
                        </div>
                      </div>
                    )}
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {rsvpList.length === 0 && <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", color: "#8b7355", textAlign: "center", padding: "12px 0" }}>No RSVPs yet</p>}
                    {rsvpList.map((r, i) => (
                      <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2" style={{ backgroundColor: "#3d2a1a" }}>
                        <div className="w-8 h-8 rounded-full flex items-center justify-center overflow-hidden shrink-0" style={{ border: "2px solid #8b7355", backgroundColor: "#2c1810" }}>
                          {r.avatar_url ? <img src={r.avatar_url} alt="" className="w-full h-full object-cover" /> : <span style={{ fontSize: "10px", fontWeight: 700, color: "#f5f0e8" }}>{getInitials(r.name)}</span>}
                        </div>
                        <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "14px", fontWeight: 600, color: "#f5f0e8", flex: 1 }}>{r.name}</span>
              ...
              {/* Vintage Comments */}
              <div className="mt-4 rounded-2xl p-4" style={{ backgroundColor: "#2c1810" }}>
                <div className="flex items-center justify-between mb-3">
                  <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "14px", fontWeight: 700, color: "#f5f0e8" }}>Comments</h2>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto mb-3">
                  {comments.length === 0 && (
                    <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", color: "#8b7355", textAlign: "center", padding: "12px 0" }}>No comments yet — be the first!</p>
                  )}
                  {comments.map((c, i) => (
                    <div key={i} className="rounded-xl px-3 py-2" style={{ backgroundColor: "#3d2a1a" }}>
                      <div className="flex items-center gap-2">
                        <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", fontWeight: 700, color: "#8b7355" }}>{c.user_name}</span>
                        <span style={{ fontSize: "10px", color: "#6b5a45" }}>{formatTime(c.created_at)}</span>
                      </div>
                      <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "14px", color: "#f5f0e8", marginTop: "2px" }}>{c.text}</p>
                    </div>
                  ))}
                </div>
                <div className="flex gap-2">
                  <input
                    ...
                    style={{ backgroundColor: "#3d2a1a", color: "#f5f0e8", fontFamily: "'Playfair Display', serif" }}
                  />
                  <button onClick={sendComment} className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: "#8b7355" }}>
              ...
              {/* Vintage Gallery */}
              <div className="mt-4 rounded-2xl p-4 mb-24" style={{ backgroundColor: "#2c1810" }}>
                <div className="flex items-center justify-between mb-3">
                  <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "14px", fontWeight: 700, color: "#f5f0e8" }}>Gallery</h2>
                  <button onClick={() => photoInput.current?.click()} className="rounded-full px-3 py-1" style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", fontWeight: 700, backgroundColor: "#8b7355", color: "#f5f0e8" }}>
                    Add photo
                  </button>
                  <input ref={photoInput} type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
                </div>
                {uploadingPhoto && <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", color: "#8b7355", textAlign: "center", padding: "8px 0" }}>Uploading...</p>}
                {photos.length === 0 && !uploadingPhoto ? (
                  <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "12px", color: "#8b7355", textAlign: "center", padding: "16px 0" }}>No photos yet — add the first one!</p>
                ) : (
